...
const getMagicMiddleware = ({ userContext, traceIdSource, packageVersion, rules, rulesFunc }) => (
  req,
  res,
  next
) => {
  ...
  let boundFinisher = api.bindFunctionToTrace((response, context) => {
    ...
    const duration = process.hrtime(context.stack[0].startTimeHR);
    const durationMs = (duration[0] * 1e9 + duration[1]) / 1e6;
    const filteredRule = _.find(rules, rule => {
      const { condition } = rule;

      return rulesFunc[condition](condition, req, response, durationMs);
    });
    const sampleRate =
      (filteredRule && filteredRule.sampleRate) ||
      parseInt(process.env["HONEYCOMB_SAMPLERATE"]) ||
      1;
    if (Math.random() < 1 / sampleRate) {
      ...
      api.finishTrace(trace, sampleRate);
    }
  });
...
let instrumentExpress = function(express, opts = {}) {
  ...
  rules.forEach(rule => {
    ...
    rulesFunc[condition] =
      rulesFunc[condition] ||
      vm.runInNewContext(`(condition, req, res, durationMs) => ${condition}`);
  });
  ...
  const wrapper = function() {
    ...
    app.use(getMagicMiddleware({ userContext, traceIdSource, packageVersion, rules, rulesFunc }));
