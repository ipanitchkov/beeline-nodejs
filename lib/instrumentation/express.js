...
const getMagicMiddleware = ({ userContext, traceIdSource, packageVersion, rules, rulesFunc }) => (
  ...
  const filteredRule = _.find(rules, rule => {
    ...
    return rulesFunc[condition](key, value, req, res);
  });
  const sampleRate = (filteredRule && filteredRule.sampleRate) || 1;
...
let instrumentExpress = function(express, opts = {}) {
  ...
  rules.forEach(rule => {
    ...
    rulesFunc[condition] =
      rulesFunc[condition] ||
      vm.runInNewContext(
        `(key, value, req, res) => req[key] ${condition} value || res[key] ${condition} value`
      );
  });
